...
public:
	int x, y;
	double param; // значение точки на числовой окружности (за центр вз€т центр фигуры)
	bool isConnectToNext; // при вызове Figure.paint соедин€ть текущюю точку линией со следующей
	Point() {
		this->x = 0;
		this->y = 0;
		this->param = 0;
...
public:
	...
	/* «адание точки на окружности фигуры */
	...
	/* ”становка значений существующей точки*/
	...
	/* —оединен€ет точеки лини€ми */
	void paint() {
		for (int i = 0; i < this->ps.size(); i++) {
			// ≈сли точка не соедин€етс€ со следующей (в случае последней точки - с первой)
			...
			// ≈сли точка между первой и последней
			else {
...
leftmostBorder = 130; // крайн€€ лева€ граница расположени€ дл€ фигуры
bool flag_shift = false; // переключатель на кнопку SHIFT (зеркальное отражение фигуры относительно себ€)
		...
		case WM_PAINT: {
			...
			if (figures.size()) {
				...
				*/
				...
				// ≈сли найден хоть один заступ по х/у - сместить центр фигуры на размер наиб. заступов по х/у
